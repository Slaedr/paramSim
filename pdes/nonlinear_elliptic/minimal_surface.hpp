...
/**
 * Describes the FEM procedures for the minimal surface equation,
...
public:
    ...
    MinimalSurface(std::shared_ptr<const Case<dim>> test_case,
...
private:
    /// Evaluates the PDE residual at one quadrature point and adds it to an
    /// output vector.
    void evaluate_point_residual(
        const FEValues<dim>& fe_values,
        const std::vector<Tensor<1, dim>>& solution_gradients, const unsigned q,
        Vector<double>& elem_rhs) const
    {
        const unsigned dofs_per_cell = fe_.n_dofs_per_cell();
        const double coeff =
            1.0 / std::sqrt(1 + solution_gradients[q] * solution_gradients[q]);
