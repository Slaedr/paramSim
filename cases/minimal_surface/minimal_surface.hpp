...
public:
    Dirichlet()
      : a(get_coeffs(params_.ac, params_.center))
    { }
...
protected:
    ...
    std::array<double, n_terms> get_coeffs(const std::array<double, n_indep_coeffs>& arr, const double y0)
    {
        double a1 = 0.0;
        for(int i = 0; i < n_indep_coeffs; i++) {
            a1 += arr[i] * (std::pow(-1.0, i)*std::pow(y0+1.0, i+2) - std::pow(1.0-y0, i+2));
        }
        a1 /= 2.0;
        double a0 = a1 * (y0 + 1.0);
        for(int i = 0; i < n_indep_coeffs; i++) {
