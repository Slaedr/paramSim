...
struct Params {
    ...
    Params()
    {
        // don't use initializer list to make sure other vars are
        // default-initialized
...
public:
                         ...
                         const unsigned int /*component*/ = 0) const override
    {
        ...
        for (int i = 0; i < Params<dim>::n_centers; ++i) {
            ...
            sum +=
                params_.coeffs[i] * std::exp(-x_minus_xi.norm_square() /
             ...
             const unsigned int /*component*/ = 0) const override
    {
        ...
        for (unsigned int i = 0; i < Params<dim>::n_centers; ++i) {
            ...
            sum +=
                params_.coeffs[i] * (-2 / (params_.width * params_.width) *
...
public:
                         ...
                         const unsigned int /*component*/ = 0) const override
    {
        ...
        for (int i = 0; i < Params<dim>::n_centers; ++i) {
            const Tensor<1, dim> x_minus_xi = p - params_.centers[i];
            const double arg =
                -x_minus_xi.norm_square() / (params_.width * params_.width);
            sum += params_.coeffs[i] * std::exp(arg) *
...
public:
    ...
    Neumann(const Params<dim> &params) : exact_solution(params) {}

    virtual double value_normal(const Point<dim> &p,
...
template <int dim>
class PoissonVerify final : public Case<dim>,
                            public HasExactSolution<dim>,
